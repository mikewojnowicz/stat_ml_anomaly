...
\begin{frame}{Simple example: Gaussian Hidden Markov Model}
	...
	\end{figure}  
		...
		%\item $\pi$, the initial latent state distribution
		\end{itemize}
...
%	\begin{itemize}
%	\item e.g, Bayesian Multivariate Gaussian Models 
%	\end{itemize}
%2. \alert{\emph{Bayesian latent variable models}}
%	\begin{itemize}
%	\item e.g. Bayesian Hidden Markov Model 
%	\end{itemize}
%3. \alert{\emph{Frequentist latent variable models}}
%	\begin{itemize}
%	\item e.g,  Frequentist Hidden Markov Model  
%	\end{itemize}
%\end{frame}
%
%		
...
%	
%	\includegraphics[width=.5\textwidth]{images/gaussian}
%	\end{figure}
%	
%	\begin{itemize}
%		\item $\+\theta = (\+\mu, \+\Sigma)$, the mean and covariance matrix.
%	\end{itemize}
%		
%	
%	\column{.6\textwidth}
%	\begin{figure} 
%	\textbf{Gaussian Hidden Markov Model} 
%	\includegraphics[width=.9\textwidth]{images/bivariate_gaussian_hmm_2} 
%	\end{figure}  
%	
%		\begin{itemize}
%		\small
...
3. \alert{\emph{Frequentist latent variable models}}
	\begin{itemize}
	\item  (standard) Variational Autoencoders, Bayesian Generalized Linear Mixed Effects Models, (standard) Hidden Markov Models$^*$%,  (typical) Hidden Markov Models$^*$ %/footnote{Standard HMM's are typically fit with EM, but as we will see, EM is a special case of VI.}
	\end{itemize}
\end{frame}
...
%Now that we have obtained a computable quantity (the VLBO),
...
\item \textit{Bayesian models}: The optimal variational density $q^*(\+u)$ raises the (approximate) evidence term $p(\+x)$ (the term used for Bayesian model comparison) as high as possible. %(So it would seem to provide a "good" distribution over $\+u$.) 
...
%
%VI has some known deficiencies.  Following is one. 
%
%\end{frame}
...
\subsection{Tool: MF-CAVI Updates On Exponential Family Complete Conditionals}
...
We can write the density of the Dirichlet distribution in exponential form.  If we let $Z(\alpha)$ be its normalizing constant (in particular, the multivariate beta function), then we have 
\begin{align*}
p \cond\alpha &= \df{1}{Z(\alpha)} p_1^{\alpha_1 -1} \cdot \cdot \cdot p_K^{\alpha_K -1} \\
&= \exp \bigg\{ \sum_{k=1}^K (\alpha_k -1) \log p_k - \log Z(\alpha) \bigg\}
\end{align*}
with natural parameter $\eta(\alpha) = [\alpha_1 -1, ..., \alpha_K -1]^T$, sufficient statistics $\log p = [\log p_1, ..., \log p_K]^T$, base measure $h(p)=1$, and normalizing constant $a(\alpha) = \log Z(\alpha)$.
\end{frame}

\begin{frame}{MF-CAVI updates on random variables with exponential family complete conditionals}  \label{slide_claim}
\textbf{Claim} Consider a model with unobserved random variables $(u_1,...,u_k)$.  Let 

\begin{enumerate}
\item Variational density $q$ have mean field factorization $q=q_k(u_k)q_{-k}(u_{-k})$. 
\item $p(u_k \cond u_{-k}, x)$ be in some parametric exponential family $\mathcal{E}$ with natural parameter $\eta_k(u_{-k}, x)$.  
\end{enumerate}
...
\begin{align}
\label{eqn:efcc}
 p(u_k \cond u_{-k}, x) = h(u_k) \exp \{ \eta_k^T t(u_k) - a(\eta_k)\} 
 \end{align}
...
\begin{align*}
q_k(u_k) & \propto  \exp \bigg\{ \E_{q_{-k}} \bigg[  \eta_k^T t(u_k) + \log h(u_k) \bigg] \bigg\} \\
& \propto  h(u_k) \exp \bigg\{ \E_{q_{-k}}[ \eta_k]^T t(u_k) \bigg\} 
\end{align*}

Since $h$ and $t$ are identical to those of \eqref{eqn:efcc}, the claim holds. 
