...
1. \alert{\emph{Bayesian latent variable models}}
	...
	\end{itemize}
2. \alert{\emph{Frequentist latent variable models}}
	\begin{itemize}
	\item  Variational Autoencoders \tiny (the classical kind) \normalsize, Bayesian Generalized Linear Mixed Effects Models, %(standard) Hidden Markov Models$^*$%,  (typical) Hidden Markov Models$^*$ %/footnote{Standard HMM's are typically fit with EM, but as we will see, EM is a special case of VI.}
	\end{itemize}
3. \alert{\emph {Bayesian (non-latent variable) models}}
	\begin{itemize}
	\item Many hierarchical Bayesian models,  Any model with non-conjugate prior, 
	\end{itemize}
\end{frame}
...
Suppose that we construct variational density with the factorization  
...
\subsection{Exponential Family Complete Conditionals}
...
\begin{itemize}
\item Encodes assumptions
\item Defines a factorization of the joint distribution
\end{itemize}
...
\footnotesize
\begin{equation}
p(\+z, \+\theta, \+w, \+\beta \cond \+\alpha, \+\eta)  = \ds\prod_{k=1}^K \explainterm{Dirichlet}{p(\+\beta_k \cond \+\eta)}  \ds\prod_{d=1}^D \explainterm{Dirichlet}{p(\+\theta_d \cond \+\alpha)} \; \ds\prod_{n=1}^N \explainterm{Categorical}{p(\+z_n \cond \+\theta)} \; \explainterm{Categorical}{p(\+w_n \cond \+z_n, \+\beta)}
\label{lda_joint}
\end{equation}
\end{frame}
...
\begin{itemize}
\item Choose $V$ vocabulary words, $K$ topics. 
\item $\eta \in \R^V, \alpha \in \R^K$ are \textit{hyperparameters}.
\item For $k$ in $(1,K)$:
	\begin{itemize}
	\item Choose \textit{per-corpus topic distribution} $\beta_k \in \R^V \sim \text{Dir}(\eta)$
	\end{itemize}
\item For $d$ in $(1,D):$
	\begin{itemize}
	\item Choose \textit{per-document topic proportions}   $\theta_d \in \R^K \sim \text{Dir}(\alpha)$
	\item For $j$ in $(1, N_d)$:
		\begin{itemize}
...
\item The parameter $\alpha$ controls the shape and sparsity of $\theta$
	\begin{itemize}
	\item high $\alpha$ = uniform $\theta$, small $\alpha$ = sparse $\theta$
	\end{itemize}
\end{itemize}
