...
\[ \soft_{ik}^{(j+1)} = \df{ c_k^{(j)}  p(x_i \cond \theta_k^{(j)}) }{ \sum_{l=1}^K c_l^{(j)}  p(x_i \cond \theta_l^{(j)})} \quad \propto\quad  \underset{prior}{c_k^{(j)}} \;\;\; \underset{likelihood}{p(x_i \cond \theta_k^{(j)})}\]
...
\label{eqn:em_algorithm_appendix}
 ...
 \end{equation}
 \vfill
\tiny \bf{Discuss:} What does and doesn't make sense about this algorithm? \\ % Expected: We need to marginalize out z.  Should make sense.   But (1) Why marginalize against that particular distribution on z (the posterior)?  (2) Where does this particular maximize-expectation form come from.
\pause 
...
\label{eqn:exponential_family_complete_data_likelihood}
 p(\+x, \+z \cond \theta) = \ds\prod_{i=1}^n h(x_i, z_i) \exp \big\{ \eta(\theta)^T \sum_{i=1}^n s(x_i, z_i) - n \,a(\eta(\theta))\big\} 
 \end{align}
...
\[ \mu(\theta^{(t+1)}) = \df{1}{n} \ds\sum_{i=1}^n   \E_{p(\+z \cond \+x , \+\theta^{(t)})} s(x_i, z_i) \]
where $\mu := \E[s(x_1,z_1)]$ refers to the mean parametrization of the likelihood.
